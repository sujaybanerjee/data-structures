...
public class TwosGame extends GridView {
    ...
    public boolean canCombineOrMove(NumberBlock block) {
        for (IntPoint next : block.getNeighborPoints()) {
            //
            if (this.grid.inBounds(next)) {
                NumberBlock neighbor = this.getBlock(next);
                if (neighbor == null) {
                    return true;
                } else if (neighbor.value == block.value) {
                    return true;
                } else {
    ...
    public boolean gameOver() {
        ...
        if (this.grid.actors.size() < 16) {
            return false;
        }
        for (int x = 0; x < 4; x++) {
            for (int y = 0; y < 4; y++) {
                if (this.canCombineOrMove(this.getBlock(new IntPoint(x, y)))) {
                    return false;
                }
            }
        }
