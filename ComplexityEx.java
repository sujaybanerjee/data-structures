...
public class ComplexityEx {
    ...
    static void timeListSearch() {
        ...
        List<Integer> dataToSearch = makeRandomInts(2000, -500, 800);
        List<Integer> queries = makeRandomInts(800, -1000, 1000);
        Set<Integer> foundQueries = new HashSet<>();

        long startTime = System.currentTimeMillis();

        for (int i = 0; i < queries.size(); i++) {
            if (listSearch(dataToSearch, queries.get(i)) != -1) {
                foundQueries.add(i);
            }
        }
        ...
        System.out.println(
                "Found " + foundQueries.size() + " numbers out of " + dataToSearch.size() + " random numbers in "
                        + timeDiff + " seconds. Speed = " + (dataToSearch.size() / timeDiff) + " Tests/Sec.");
